// v=force-1 — client-side mNAV from base series (Price, Shares ffilled, NAV)
console.log("mNAV Pages app loaded: v=force-1", new Date().toISOString());
...
(async function () {
  ...
  for (const sym of symbols) {
    ...
    const title = document.createElement("h2"); title.textContent = sym; card.appendChild(title);
    ...
    new Chart(canvas.getContext("2d"), {
      ...
      options: {
        ...
        scales: {
          ...
          y: { grid: { color: "#22252a" },
               ticks: { callback: (v) => Intl.NumberFormat(undefined, { notation: "compact", maximumFractionDigits: 2 }).format(v) } }
        },
        plugins: {
